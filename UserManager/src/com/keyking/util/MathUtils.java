...
//	}
	...
	/*
	 * ǿ���ɹ����ʱ�: ���д��Ŀ��ǿ���Ǽ���?
	 * ���д��ɹ���,0�д���ɹ��ʣ�?�д��?0�����˷�ӳɳɹ��ʣ�?�Ҵ��?0�����˷�ӳɳɹ��ʣ��Դ�����?
	 * ���ʷŴ� 100��
	 */
	...
	static class IntegerComparator implements Comparable<IntegerComparator>{
		
		public int id;
		
		public int rate;
		
		public IntegerComparator(int id ,int rate){
			this.id   = id;
			this.rate = rate;
		}
	...
	public static int getRandomId(int ids[],int rate[]){
		int total = 0;
		IntegerComparator[] coms = new IntegerComparator[rate.length];
		for(int i= 0;i < rate.length ; i++){
			total += rate[i];
			coms[i] = new IntegerComparator(ids[i],rate[i]);
		}
		Arrays.sort(coms);
	...
	public static int getRandomId(int ids[],int rate[],int num){
		...
		if(total > num ){//重新计算每一个的新的几率.
			for(int i= 0;i < rate.length ; i++){
				coms[i] = new IntegerComparator(ids[i],rate[i] * num / total);
			}
		}else{
			for(int i= 0;i < rate.length ; i++){
				coms[i] = new IntegerComparator(ids[i],rate[i]);
	...
	//	System.out.println(ceil(2.1));
		for(int i=0;i<100;i++){
			System.out.println(randomNoInclude(50,60,55));
		}
	}
