...
public class HttpDealFavorite extends HttpHandler {
	//http://139.196.30.53:32104/HttpDealFavorite?type=0&tid=xxx&did=xxx&content=xxx
	@Override
	public void handle(HttpRequestMessage request, HttpResponseMessage response) {
		...
		if (deal != null){
			...
			if (user != null){
				...
				String str = "{\"favorites\":" + JsonUtil.ObjectToJsonString(user.getFavorites()) + "}";
				response.appendBody(formatJosn(request,str));
			}
		}else{
			error(request,response,"未找到知道交易贴");
		}
	}
