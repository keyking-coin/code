...
public class HttpQueryOthers extends AbstractHandler{
	@Override
	public String logic(HttpRequestMessage request) {
		...
		if(isNum){
			...
			String type=request.getParameter("playerProject");
			...
			if (serList.contains(serverId)) {
				...
				int protocolId = 0x00000059; // 玩家信息
				...
				handler.addNextDo(userId, new NeedContinueDoSomthing() {
					...
					public JoyProtocol succeed(UserInfo info,
						...
						map.put("result", "ok");
						map.put("QueryOtherInfo", params.get(3));
				...
				if (!map.containsKey("result")) {
					...
					if (map.get("result").equals("ok")) {
						return  map.get("QueryOtherInfo").toString();
					} else {
