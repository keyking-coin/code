...
public class HttpQueryGenPart extends AbstractHandler{
	@Override
	public String logic(HttpRequestMessage request) {
		...
		if(isNum){
			...
			if (serList.contains(serverId)) {
				...
				int protocolId = 0x0000006F; // 查询兵科物资建装等通用
				...
				handler.addNextDo(userId, new NeedContinueDoSomthing() {
					...
					public JoyProtocol succeed(UserInfo info,
						...
						map.put("result", "ok");
						map.put("PlayerGeneralPart", params.get(3));
				...
				if (!map.containsKey("result")) {
					...
					if (map.get("result").equals("ok")) {
						return  map.get("PlayerGeneralPart").toString();
					} else {
