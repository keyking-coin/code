...
public class HttpModifyBuildLevel extends AbstractHandler {
	@Override
	public String logic(HttpRequestMessage request) {
		...
		if(isNum){
			...
			String buildSlot = request.getParameter("playerBuildSlot");
			String  number =  request.getParameter("playerParameterInput");
			...
			if (serList.contains(serverId)) {
				...
				int protocolId = 0x0000006A; // 修改玩家建筑等级
				...
				resp.getParams().put(buildSlot);
				resp.getParams().put(number);
