...
public class HttpModifyEquipOp extends AbstractHandler{
	@Override
	public String logic(HttpRequestMessage request) {
		...
		if(isNum){
			...
			final int serverId = Integer.parseInt(request.getParameter("playerServerId"));
			String equipId = request.getParameter("playerEquipId"); //装备在数据库的id
			String operation = request.getParameter("playerOperation");
			...
			if (serList.contains(serverId)) {
				...
				int protocolId = 0x0000006D; // 修改玩家装备
				...
				resp.getParams().put(equipId);
				resp.getParams().put(operation);
				...
				if (!map.containsKey("result")) {
					...
					if (map.get("result").equals("ok")) {
						return  map.get("Modify").toString();
					} else {
