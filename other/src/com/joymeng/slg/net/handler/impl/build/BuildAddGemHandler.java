...
public class BuildAddGemHandler extends ServiceHandler{
	@Override
	public void _deserialize(JoyBuffer in,ParametersEntity params) {
		params.put(in.getInt());//城市id
		params.put(in.getLong());//建筑id
		params.put(in.get());//byte 0-添加，1-获取
	...
	public JoyProtocol handle(UserInfo info,ParametersEntity params) throws Exception {
		...
		byte flag = params.get(2);
		String itemId = params.get(3);
		...
		if(comGem != null){
			if(flag == 0){
				if(!comGem.addProductionGems(role, build, itemId)){ //添加宝石生产
					resp.fail();
				}
			}else if(flag == 1){
				if(!comGem.getGems(role, build, itemId)){ //获取生产完成的宝石
					resp.fail();
				}
			}
		}else{
