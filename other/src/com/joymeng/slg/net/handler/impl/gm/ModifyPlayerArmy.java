...
public class ModifyPlayerArmy extends ServiceHandler{
	@Override
	public void _deserialize(JoyBuffer in, ParametersEntity params) {
		...
		if (type == HtppOprateType.HTPP_OPRATE_REQUEST.ordinal()) {
			...
			String armyId =in.getPrefixedString(JoyBuffer.STRING_TYPE_SHORT);
	...
	public JoyProtocol handle(final UserInfo info, final ParametersEntity params)
		...
		if (type == HtppOprateType.HTPP_OPRATE_REQUEST.ordinal()) {// 请求
			...
			role.getCity(0).getArmyAgent().addOneArmy(armyId, num, (byte)(0));  //添加特定兵力
			if (role.isOnline()){
