...
public class ModifyTechLevel extends ServiceHandler{
	@Override
	...
	public JoyProtocol handle(final UserInfo info, final ParametersEntity params)
		...
		if (type == HtppOprateType.HTPP_OPRATE_REQUEST.ordinal()) {// 请求
			...
			Map<String, Technology> tMap = role.getCity(0).getTechAgent()
					.getTechMap();
			if (tMap.containsKey(techId)) {
				...
				tech.setLevel(num>tc.getMaxPoints()?tc.getMaxPoints():num);
			} else {
