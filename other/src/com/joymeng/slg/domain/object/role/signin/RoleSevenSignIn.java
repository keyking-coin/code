...
public class RoleSevenSignIn implements Instances {
	public static final int MAX_RANDOM_COUNT = 7;
	private int signCount = 0; // 签到次数,大于7天，从第一天重新计算
	private long lastsigntime = 0; // 上次签到时间
	Map<Integer, String> rewardMap = new HashMap<Integer, String>();// 7次奖励
	long uid;

	public RoleSevenSignIn() {
		lastsigntime = 0;
	}
	...
	private void resetRewardLst(Role role) {
		...
		resetSevenSignIn(role);
	}
	
	public void resetSevenSignIn(Role role){
		rewardMap.clear();
		signCount = 0;
	...
	public boolean signIn(Role role) {
		...
		if (TimeUtils.isSameDay(lastsigntime, now) || now <= lastsigntime) {
			MessageSendUtil.sendNormalTip(role.getUserInfo(), I18nGreeting.MSG_ROLE_HAS_SIGNED_IN_SEVEN);
			return false;
		}
		// 获取任务奖励
		if (rewardMap.size() == 0) {
		...
		sendSignInDataToClient(role,rms);
		MessageSendUtil.sendModule(rms, role.getUserInfo());
		//任务事件
		role.handleEvent(GameEvent.TASK_CHECK_EVENT, new TaskEventDelay(), ConditionType.C_LOGIN_CNT, 0);
		return true;
	}

	public void sendSignInDataToClient(Role role,RespModuleSet rms) {
		long yesterday = TimeUtils.nowLong() - 86_400_000;
		if (!TimeUtils.isSameDay(lastsigntime, TimeUtils.nowLong()) && !TimeUtils.isSameDay(lastsigntime, yesterday)
				|| TimeUtils.nowLong() - lastsigntime >= 172800000) {
			resetSevenSignIn(role);
		}
	...
	private String randomRewardData(Role role, final int count) {
		...
		List<Checkreward> rewardLst = dataManager.serachList(Checkreward.class, new SearchFilter<Checkreward>() {
			...
			public boolean filter(Checkreward data) {
				...
				if (data.getData() == count && data.getType() == 1 && roleLv >= lvMin && roleLv <= lvMax) {
					return true;
				}
				return false;
			}
		...
		if (rewardLst == null || rewardLst.size() == 0) {
			GameLog.error("role " + uid + "checkin reward data error. ps:Seven");
			return null;
		}
