...
public class RoleCityAgent implements DaoData,Instances,BuildLinkQueue{
	...
	Map<Long, List<UnionHelper>> unionHelpers = new HashMap<Long, List<UnionHelper>>();// 建筑请求联盟帮助次数的记录 <建筑ID,已被帮助的用户uid列表>
	...
	public void setCityBuffs(Map<String, Map<String, BuffObject>> cityBuffs) {
		this.cityBuffs = cityBuffs;
	}

	public void addBuffInMap(String buildId, String key, byte type, int value, float rate) {
	...
	public void removeBuffInMap(String buildId, String key, byte type, int value, float rate){
		Map<String, BuffObject> buildBuff = cityBuffs.get(buildId);
		if(buildBuff == null){
			return;
		}
	...
	public synchronized byte createBuild(String bbsId,String buildingId,Role role, int money){
		...
		if (!checkPower(buildingId,initializeLevel)){
		...
		//add buff
		// 检查资源条件，使用金币
		long time = (long) (buildLevel.getTime() * (1 - cityAttr.getImpBuildSpeed()));
		int costMoney = 0;
		if(money > 0){
			if(money == 2){
		...
		if (addBuild(build)){
			...
			List<Object> costRes = redCostResource(buildLevel.getBuildCostList(), costMoney,"createBuild");
	...
	public boolean checkBuildConditions(Role role, List<String> needBuildingIDList){
		for (int i = 0 ; i < needBuildingIDList.size() ; i++){
			...
			if (!flag){
				MessageSendUtil.sendNormalTip(role.getUserInfo(),I18nGreeting.MSG_BUILD_NEED_BUILD,needLevelNum,needBuilding.getBuildingName());
				return false;
			}
		}
		return true;
	...
	public synchronized byte BuildLevelup(Role role, long buildId, int money) {
		...
		// 检查建筑等级对应的升级条件
		if (!checkPower(roleBuild.getBuildId(), newLevel)) {
			MessageSendUtil.sendNormalTip(role.getUserInfo(), I18nGreeting.MSG_BUILD_NO_POWER);
			return ErrorCodeConst.ERR_NORMAL.getKey();
		}
		...
		// add buff
		long time = (long) (buildLevel.getTime() * (1.0f - cityAttr.getImpBuildSpeed()));
		...
		if (money == 1) {
			role.redRoleMoney(costMoney);
			role.sendRoleToClient(rms);
			String event = "BuildLevelup";
			LogManager.goldConsumeLog(role, costMoney, event);
			roleBuild.leveupFinish(true);
			if (costMoney > 0) {
				// 任务事件
				role.handleEvent(GameEvent.TASK_CHECK_EVENT, new TaskEventDelay(), ConditionType.C_ACC_BUILD,TimerLastType.TIME_LEVEL_UP);
		...
		List<Object> costRes = redCostResource(buildLevel.getBuildCostList(), costMoney,"BuildLevelup");
		role.sendResourceToClient(false, rms, id, costRes.toArray());
	...
	public boolean cancelCreateBuild(Role role, long buildId){
		...
		for (int i = 0; i < items.size(); i++) {
			...
			role.getBagAgent().addGoods(params[0],Integer.valueOf(params[1]));
			LogManager.itemOutputLog(role, Integer.valueOf(params[1]), "cancelCreateBuild", params[0]);
		}
    	LogManager.buildLog(role, roleBuild.getSlotID(),roleBuild.getBuildId(),roleBuild.getLevel(), BuildOperation.cancleCreBuild.getKey());
		...
		for (int i =0 ; i < costObjs.length ; i += 2){
			...
			String event ="cancelCreateBuild";
	...
	public boolean cancelBuildLevelup(Role role, long buildId) {
		...
		if (bCancel) {
			...
			for (int i = 0; i < items.size(); i++) {
				...
				role.getBagAgent().addGoods(params[0],Integer.valueOf(params[1]));
				LogManager.itemOutputLog(role, Integer.valueOf(params[1]), "cancelBuildLevelup", params[0]);
			}
			LogManager.buildLog(role, roleBuild.getSlotID(), roleBuild.getBuildId(), roleBuild.getLevel(),
			...
			RespModuleSet rms = new RespModuleSet();
			sendToClient(rms, false);// 发城市
			role.getBagAgent().sendBagToClient(rms);//下发背包
			...
			for (int i = 0; i < costObjs.length; i += 2) {
				...
				String event = "cancelBuildLevelup";
				String item = type.getKey();
				if (value != 0) {
					LogManager.itemOutputLog(role, value, event, item);
				}
	...
	public boolean cancelRemoveBuild(Role role, long buildId){
		...
		if(bCancel){
			...
			roleBuild.sendToClient(rms);//下发建筑
			MessageSendUtil.sendModule(rms, role.getUserInfo());
			LogManager.buildLog(role,roleBuild.getSlotID(), roleBuild.getBuildId(),roleBuild.getLevel(), BuildOperation.cancleReBuild.getKey());
		}else{
			MessageSendUtil.sendNormalTip(role.getUserInfo(),I18nGreeting.MSG_BUILD_NOT_FIND, buildId);
			return false;
		}
	...
	public byte removeBuild(Role role, long buildId, int money){
		...
		if (!couldUseBuildQueue(time)) {
			if (checkTimeQueueState() && checkBuildQueueTime(time)) {
				MessageSendUtil.sendNormalTip(role.getUserInfo(), I18nGreeting.MSG_NO_BUILD_QUEUE);
				return ErrorCodeConst.ERR_QUEUE_LMT.getKey();
			} else {
				MessageSendUtil.sendNormalTip(role.getUserInfo(), I18nGreeting.MSG_TIME_BUILD_QUEUE);
		...
		int costMoney = 0;
		if(money > 0){
			costMoney = getCostMoney(role, buildLevel.getBuildCostList(),buildLevel.getNeeditem(), buildLevel.getTime(), (byte)0);
			if(costMoney > role.getMoney()){
				MessageSendUtil.sendNormalTip(role.getUserInfo(),I18nGreeting.MSG_ROLE_NO_MONEY,costMoney);
				return ErrorCodeConst.ERR_NORMAL.getKey();
			}
		}
		LogManager.buildLog(role, roleBuild.getSlotID(),roleBuild.getBuildId(),roleBuild.getLevel(), BuildOperation.removeBuild.getKey());
	...
	public boolean changeBuildsSlot(Role role, String slotId1, String slotId2){
		...
		if(build1 != null || build2 != null){
			if(build1 != null){
				...
				LogManager.buildLog(role, build1.getSlotID(),build1.getBuildId(),build1.getLevel(), BuildOperation.moveBuild.getKey());
			}
			if(build2 != null){
				build2.setSlotID(slotId1);
			    LogManager.buildLog(role, build2.getSlotID(),build2.getBuildId(), build2.getLevel(),BuildOperation.moveBuild.getKey());
			}
	...
	public void sendCityBuffToClient(RespModuleSet rms) {
		...
		//RoleCityAttr buff
//		module.add(String.valueOf(cityAttr.getAddSoldLimit())); //float 单支部队的兵力数量上限
		rms.addModule(module);
	}
	 ...
	 * @return
	 */
	public boolean checkPower(String buildKey,int level){
		Building building = dataManager.serach(Building.class,buildKey);
		if (building != null && building.getBuildingComponent().contains(BuildComponentType.BUILD_COMPONENT_ELECTRICAL.getKey())){
			return true;
		}
		...
		for (int i = 0; i < builds.size(); i++) {
			...
			case FOOD_FACT:
				if (resMap == null || resMap.get(ResourceTypeConst.RESOURCE_TYPE_FOOD) == null) {
					...
					production = com.getOutput() + resMap.get(ResourceTypeConst.RESOURCE_TYPE_FOOD);
				}
			...
			case SMELTER:
				if (resMap == null || resMap.get(ResourceTypeConst.RESOURCE_TYPE_METAL) == null) {
					...
					production = com.getOutput() + resMap.get(ResourceTypeConst.RESOURCE_TYPE_METAL);
				}
			...
			case REFINERY:
				if (resMap == null || resMap.get(ResourceTypeConst.RESOURCE_TYPE_OIL) == null) {
					...
					production = com.getOutput() + resMap.get(ResourceTypeConst.RESOURCE_TYPE_OIL);
				}
			...
			case TITANIUM_PLANT:
				if (resMap == null || resMap.get(ResourceTypeConst.RESOURCE_TYPE_ALLOY) == null) {
					...
					production = com.getOutput() + resMap.get(ResourceTypeConst.RESOURCE_TYPE_ALLOY);
				}
	...
	public int getMaxTrainNum(BuildName name){
		...
		if(BuildName.MILITARY_FACT == name){
			...
			return maxTrainNum;//(int) (maxTrainNum + maxTrainNum*cityAttr.getAddSProdLimit());
		}
	...
	public long getMaxLootResource(ResourceTypeConst type) {
		...
		// add buff
		if (maxResource > 0) {
			long resProtectNum = (long) (maxResource*(1 + cityAttr.getImpProtect()/* + cityAttr.getAddStorageLimit()*/));
	...
	public int getFenceCurTrip(){
		...
		for (int i = 0 ; i < armys.size() ; i++){
			...
			if (army.getArmyBase().getArmyType() == ArmyType.HOOK.ordinal() && army.getState() == ArmyState.ARMY_IN_NORMAL.ordinal()){
	...
	public int getFenceMaxTrip(){
		int value = 0;
		List<RoleBuild> builds = searchBuildByBuildId(BuildName.FENCE.getKey());//物流中心
		...
		if (resSyncTime > 0) {
			...
			if (deducGrain != 0) { // 部队需要消耗粮食才下发
				...
				if (grain == 0) {
					...
					rms.addModule(module);
					MessageSendUtil.tipModule(rms, MessageSendUtil.TIP_TYPE_NORMAL,I18nGreeting.MSG_FOOD_SHORTAGE_FORCE_ARMY);
					...
					String event = "";
					if (isOnline) {
						event = "grainConsumption";
					} else {
						event = "getOutlineConsumption";
					}
					String item = type.getKey();
					LogManager.itemConsumeLog(role, grain, event, item);
					try {
					...
					String item = type.getKey();
					LogManager.itemConsumeLog(role, (long) deducGrain, event, item);
					try {
						NewLogManager.baseEventLog(role, "arms_consume", item, grain);
					} catch (Exception e) {
						GameLog.info("埋点错误");
					...
					MessageSendUtil.sendNormalTip(role.getUserInfo(), I18nGreeting.MSG_MAINTAIN_UNIT_CONSUM,deducGrain);
				}
				MessageSendUtil.sendModule(rms, role.getUserInfo());
			}
		}
		...
		if (armyLst != null){
			...
			for(ArmyInfo armyInfo : armyLst){
				...
				for(String cost : costLst){
					...
					if(role != null){
						buff = role.getArmyAttr().getEffValV2(TargetType.T_A_RED_SC, ExtendsType.EXTEND_ARMY, armyBase.getArmyType());
					}
	...
	public boolean roleTradeCityResource(Role role, String type1, long num1, String type2, long num2){
		...
		//扣税
		Buildinglevel buildLevel = RoleBuild.getBuildinglevelByCondition(tradeBuild.getBuildId(), tradeBuild.getLevel());
		if(buildLevel == null){
			MessageSendUtil.sendNormalTip(role.getUserInfo(), I18nGreeting.MSG_ROLE_CHG_RES_ERROR);
			return false;
		}
		...
		long cdTime = Long.parseLong(paramList.get(0));
		float taxRate = Float.parseFloat(paramList.get(1));
		long maxTradeNum = Long.parseLong(paramList.get(2));
		if(num1 > maxTradeNum){
			MessageSendUtil.sendNormalTip(role.getUserInfo(), I18nGreeting.MSG_ROLE_CHG_RES_ERROR);
			return false;
		}
     	num2 = (long) Math.round(num1*(1-taxRate));
		if (Math.abs(num2 - compare) >= 1) {
			num2 = compare;
		}
		resources.put(resType1, getResource(resType1) - num1);
		resources.put(resType2, getResource(resType2) + num2);
		LogManager.itemConsumeLog(role, num1, "roleTradeCityResource",resType1.getKey());
		LogManager.itemOutputLog(role, num2, "roleTradeCityResource",resType2.getKey());
		try {
...
//		long totalNum=0;
//		//检查交易前后的资源是否相等
//		for (int i =0 ; i < ress.length ; i += 2){
//			ResourceTypeConst type = ResourceTypeConst.search(String.valueOf(ress[i]));
//			if(type == null){
//				MessageSendUtil.sendNormalTip(role.getUserInfo(), I18nGreeting.MSG_ROLE_CHG_RES_ERROR);
//				return false;
//			}
//			long value = ((Number)ress[i + 1]).longValue();
//			if (value <= 0){
//				MessageSendUtil.sendNormalTip(role.getUserInfo(), I18nGreeting.MSG_ROLE_CHG_RES_ERROR);
//				return false;
//			}
...
//				return false;
//			}
//			nowTotalNum += value*resType.getWeight();
//			totalNum += getResource(type)*resType.getWeight();
//		}
//		if(Math.abs(nowTotalNum - totalNum) > 5){
//			MessageSendUtil.sendNormalTip(role.getUserInfo(), I18nGreeting.MSG_ROLE_CHG_RES_ERROR);
//			return false;
//		}
//		for (int i =0 ; i < ress.length ; i += 2){
//			ResourceTypeConst type = ResourceTypeConst.search(String.valueOf(ress[i]));
//			long value = ((Number)ress[i + 1]).longValue();
//			if(value > getResource(type)){
//				value = (long) (value - (value - getResource(type))*0.3);
//			}
//			resources.put(type, getResource(type) - value);
//		}
	}
	...
	public void setWallBuff(boolean isRemove, int value){
		...
		if(com != null){
			com.updateWallHP(isRemove, value);
		}
	}
	 ...
	 * @param buildId
	 */
	public void updateProductionBuff(Effect e, boolean isRemove, String buildId) {
		if(buildId != null){
	...
	public boolean getAllResDetails(Role role){
		...
		Map<ResourceTypeConst, Long[]> resMap = new HashMap<ResourceTypeConst, Long[]>();
		Long[] resBuff1 = new Long[]{0L,0L,0L,0L,0L,0L,0L};
		resMap.put(ResourceTypeConst.RESOURCE_TYPE_FOOD, resBuff1);
		Long[] resBuff2 = new Long[]{0L,0L,0L,0L,0L,0L,0L};
		resMap.put(ResourceTypeConst.RESOURCE_TYPE_METAL, resBuff2);
		Long[] resBuff3 = new Long[]{0L,0L,0L,0L,0L,0L,0L};
		resMap.put(ResourceTypeConst.RESOURCE_TYPE_OIL, resBuff3);
		Long[] resBuff4 = new Long[]{0L,0L,0L,0L,0L,0L,0L};
		...
		for (int i = 0 ; i < builds.size() ; i++){
			...
			if (bSuc) {
				...
				if (com.getSpecialTimer() == null) {
					...
					resBuffDet[0] += com.getOutput();
					resBuffDet[1] += com.getBaseOutput();
					resBuffDet[2] += com.getItemBuffOutput() + com.getTechBuffOutput() + com.getSkillBuffOutput()
							+ com.getVipBuffOutput() + com.getEquipBuffOutput();
