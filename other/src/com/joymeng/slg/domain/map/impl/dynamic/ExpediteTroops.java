...
public class ExpediteTroops extends AbstractObject implements TimerOver,SerializeEntity {
	...
	public void serialize(JoyBuffer out) {
		...
		out.putPrefixedString(String.valueOf(speed),JoyBuffer.STRING_TYPE_SHORT);//string 行军速度
	...
	public void startFight(MapObject obj,long now){
		...
		LogManager.mapLog(role, startPosition,targetPosition, id, "startFighting");
	}
	...
	public void finish() {
		...
		try {
			...
			if (!isReturn){
				//sendToClient(ARRIVE_DO_FIGHT_END);
				//remove();//移除行军数据
				for (int i = 0 ; i < teams.size() ; i++){
					TroopsData troops = teams.get(i);
					Role role = world.getRole(troops.getInfo().getUid());
					role.handleEvent(GameEvent.TROOPS_SEND);
					if (troops.isLeader() && timer.getType() == TimerLastType.TIME_EXPEDITE_FIGHT){
						role.handleEvent(GameEvent.UNION_FIGHT_CHANGE,false);
					}
				}
				MapObject target = mapWorld.searchObject(targetCell);
				if (target != null && target.getFight() == id && fighting){
	...
	public void goBackToCome(Map<String,Integer> resources){
		...
		}else if (targetCell.getType() == MapCellType.MAP_CELL_TYPE_FORTRESS || targetCell.getType() == MapCellType.MAP_CELL_TYPE_BARRACKS){
		...
		if (callBack){//召回逻辑
			...
			if (timer.getType() == TimerLastType.TIME_EXPEDITE_CREATE_MOVE || timer.getType() == TimerLastType.TIME_EXPEDITE_CREATE_FORTRESS){
			...
			for (int i = 0 ; i < teams.size() ; ){
				...
				long castTime = MapUtil.computeCastTime(expedite.startPosition,expedite.targetPosition,expedite.speed);//行军需要的时间
		...
		if (resources != null && resources.size() > 0){//计算每个成员获得战利品
			...
			for (int k = start ; k <= end ; k++){
				...
				int save = have;
				for (int i = 0 ; i < troopsDatas.size() ; i++){
					TroopsData old    = teams.get(i);
					TroopsData troops = troopsDatas.get(i);
					float weight = weights.get(i);
					Role role = roles.get(i);
					float nowWeight = troops.computeWeight(role);
					if (nowWeight > 0 && have > 0){//自己还有负重，并且资源还没被瓜分完
						float rate = weight / totalWeight;//按自己负重在总负重战的比例来算
						int getNum = (int)(save * resType.getWeight() * rate);
						getNum = Math.min(getNum,(int)nowWeight);//取获得和自己负重的最小值
						getNum /= resType.getWeight();//实际抢到的资源
						have -= getNum;
						...
						if (getNum != 0) {
							LogManager.pvpLog(role, group_id, (byte) 1,rtc.getKey(),getNum);
						}
					}
				}
	...
	public void packageBack(RespModuleSet rms,Role role){
		for (int i = 0 ; i < teams.size() ; i++){
			...
			troops.addResourceToCity(role,changes,objs,"packageBack");
			if (changes.size() > 0){
				role.getBagAgent().sendItemsToClient(rms,changes);//背包变化的道具修改
			}
			if (objs.size() > 0){
				role.addResourcesToCity(false,rms,troops.getInfo().getCityId(),objs.toArray());
			}
